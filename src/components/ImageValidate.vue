...
const onCheckFileUpload = (event) => {
  ...
  if (convertByteToMB(event.target.files[0].size) > Number(props?.imageProps?.maxSize)) {
    setErrors(`Dung lượng tệp không được vượt quá ${props?.imageProps?.maxSize}MB`)
    return true
  }
...
const onUpload = async (event) => {
  ...
  resizeImage(event.target.files[0], maxSize, (resizedBlob) => {
    ...
    <div class="flex flex-column gap-1">
      ...
      >
        ...
        <div
          ...
          :class="[props.class, 'relative flex justify-content-center align-items-center']"
        >
          <div class="flex flex-column gap-1">
            <ProgressSpinner
        ...
        <!-- Xem trước -->
        <div v-if="value && !isLoading" :class="[props.class, 'relative']" :style="props.style">
          <i
          ...
          <video v-if="props.imageProps.accept === 'video/*'" class="w-full h-full" controls>
