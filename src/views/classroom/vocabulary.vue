...
const onClickIsAll = (type) => {
  const newValue =
    type === "isExample" ? !isAllExample.value : !isAllTranslate.value;
...
const handleScroll = () => {
  const lessonIds = lesson.value.map((lesson) => ({
    id: lesson.id,
    top: document.getElementById("lesson-" + lesson.id)?.getBoundingClientRect()
      .top,
  }));
...
<template>
  <div class="flex gap-2">
    ...
    <div
      class="mt-4 md:mt-7 py-4 md:pt-2 flex-1 pl-2"
      style="margin-inline: auto"
    >
      <div
        class="flex flex-column gap-3 mx-0 md:mx-8"
        style="margin-inline: auto"
      >
